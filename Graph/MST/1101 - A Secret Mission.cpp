...
void mst(int n)
{
    ...
    i64 count = 0, s = 0;
    fr(edges.size()){
        int x = finds(edges[i].u);
        int y = finds(edges[i].v);
        int w = edges[i].w;

        if (x != y) {
            v[edges[i].u].pb(edges[i].v);
            v[edges[i].v].pb(edges[i].u);

            weight[mp(edges[i].u,edges[i].v)]= w;
            weight[mp(edges[i].v,edges[i].u)]= w;
...
{
    int tmp, log, i, maxi= 0, mini= 1e6+6;
