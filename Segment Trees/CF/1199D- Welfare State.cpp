...
int lazy[4*maxn], n, l[maxn], r[maxn], a[maxn];
...
void fix(int at, int l, int r, int pos, int val){
    if(l==r){
        lazy[at]= val;
        return;
    }
    ...
    if(pos<=mid)  fix(lefts,l,mid,pos,val);
    else  fix(rights,mid+1,r,pos,val);
}
...
main(){
    ...
    while(opt--){
        ...
        if(x==1){
            ...
            si(y);
            update(1,0,n-1,0,n-1,y);
        }

    }

    fr(n){
        outis(query(1,0,n-1,i));
    }
}
