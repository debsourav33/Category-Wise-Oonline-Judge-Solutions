...
i64 sub_hash(int cnt, int i, int j){
...
void solve(){
    ...
    fr1(n){
        if(n-i-i>=0 && sub_hash(1,0,i-1)==sub_hash(0,n-i-i,n-1-i))
            ans= 2*i;
        else if(n-i-i+1>=0 && sub_hash(1,0,i-1)==sub_hash(0,n-i-i+1,n-i))
            ans= 2*i-1;
    }
    ...
    fr(n)  printf("%c",s[i]);
    for(int i=n-1-ans;i>=0;i--)  printf("%c",s[i]);
    puts("");
}
